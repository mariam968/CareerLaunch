import { openDpModal } from "./modal.js";
const SHORTCUT_ROWS = [
    { keys: "+ / =", action: "Zoom in" },
    { keys: "-", action: "Zoom out" },
    { keys: "F", action: "Fit width" },
    { keys: "P", action: "Fit page" },
    { keys: "← / →", action: "Previous / next page" },
    { keys: "Home / End", action: "First / last page" },
    { keys: "Ctrl+F / /", action: "Search in document" },
    { keys: "T", action: "Toggle thumbnails" },
    { keys: "?", action: "Show this panel" }
];
function isEditableTarget(target) {
    if (!(target instanceof HTMLElement)) {
        return false;
    }
    const tag = target.tagName;
    return tag === "INPUT" || tag === "TEXTAREA" || tag === "SELECT" || target.isContentEditable;
}
export function openPdfKeyboardShortcutsModal(host) {
    const list = globalThis.document.createElement("dl");
    list.className = "dp-shortcuts-list";
    for (const row of SHORTCUT_ROWS) {
        const dt = globalThis.document.createElement("dt");
        dt.textContent = row.keys;
        const dd = globalThis.document.createElement("dd");
        dd.textContent = row.action;
        list.append(dt, dd);
    }
    openDpModal(host, {
        title: "Keyboard shortcuts",
        description: "Shortcuts work while the PDF viewer is focused (not inside a text field).",
        content: list,
        actions: [{ label: "Close", variant: "primary", onClick: () => undefined }]
    });
}
export function attachPdfKeyboardShortcuts(root, actions) {
    const onKeyDown = (event) => {
        if (isEditableTarget(event.target)) {
            return;
        }
        const key = event.key;
        if (key === "?" && !event.ctrlKey && !event.metaKey && !event.altKey) {
            event.preventDefault();
            openPdfKeyboardShortcutsModal(root);
            return;
        }
        if (key === "+" || key === "=") {
            event.preventDefault();
            actions.onZoomIn();
            return;
        }
        if (key === "-") {
            event.preventDefault();
            actions.onZoomOut();
            return;
        }
        if (key === "f" || key === "F") {
            event.preventDefault();
            actions.onFitWidth();
            return;
        }
        if (key === "p" || key === "P") {
            event.preventDefault();
            actions.onFitPage();
            return;
        }
        if (key === "ArrowLeft") {
            event.preventDefault();
            actions.onPrevPage();
            return;
        }
        if (key === "ArrowRight") {
            event.preventDefault();
            actions.onNextPage();
            return;
        }
        if (key === "Home") {
            event.preventDefault();
            actions.onFirstPage();
            return;
        }
        if (key === "End") {
            event.preventDefault();
            actions.onLastPage();
            return;
        }
        if ((event.ctrlKey || event.metaKey) && key.toLowerCase() === "f") {
            event.preventDefault();
            actions.onToggleSearch?.();
            return;
        }
        if (key === "/" && !event.ctrlKey && !event.metaKey) {
            event.preventDefault();
            actions.onToggleSearch?.();
            return;
        }
        if (key === "t" || key === "T") {
            event.preventDefault();
            actions.onToggleThumbnails?.();
        }
    };
    root.addEventListener("keydown", onKeyDown);
    return () => root.removeEventListener("keydown", onKeyDown);
}
//# sourceMappingURL=pdf-keyboard-shortcuts.js.map